...
- `app.js` contains all current quiz content and client-side behavior. `questions` is the source data for the current 25 questions; each entry contains the prompt, four options, the official answer letter, and the main explanation. `incorrectExplanations` is a parallel array containing the per-option explanations. A multi-year implementation should move these arrays behind a year/exam-set data model rather than duplicating rendering logic.
